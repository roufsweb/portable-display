...
### 4. Golden Working Custom Firmware:
- **Production Binary**: [PCB800869_PROART_FHD_FIRMWARE.bin](file:///e:/rouf/hardware-project/portable%20display/PCB800869_PROART_FHD_FIRMWARE.bin) (**589,824 bytes / 9 Banks**)
- **Archive Copy**: [03_Firmware/PCB800869_PROART_FHD_FIRMWARE.bin](file:///e:/rouf/hardware-project/portable%20display/03_Firmware/PCB800869_PROART_FHD_FIRMWARE.bin)
- **Resolved Issues & Features**:
  1. **Smooth 100-Step PWM Backlight**: Eliminates brightness jump/sticking; quick menu scales smoothly 0%–100%.
  2. **DCR & Main OSD Menu Unlocked**: Disabling DCR (Dynamic Contrast Ratio) unlocks manual Brightness & Contrast in the OSD.
  3. **Custom EDID Branding (`ProArt FHD`)**: Both Type-C and HDMI EDIDs display `ProArt FHD` in Windows, macOS, and NVIDIA Control Panel with valid checksums.
  4. **English Default**: Pre-configured with English as the permanent default language (VCP `0xCA = 2`).
- **Cleaned Up Workspace**: All intermediate non-working 512KB binaries were purged; only the original untouched [full-backup.bin](file:///e:/rouf/hardware-project/portable%20display/full-backup.bin) and the production [PCB800869_PROART_FHD_FIRMWARE.bin](file:///e:/rouf/hardware-project/portable%20display/PCB800869_PROART_FHD_FIRMWARE.bin) are maintained.
